...
export class modes extends Component {
    ...
    loginScreenRef: any = null;
    onLoad() {
        this.loginScreenRef = DataHandler.Instance.loginScreen.getComponent(loginscreen);
        ...
        this.backButton.on(Input.EventType.TOUCH_START, () => {
            this.loginScreenRef.backFromModeSelection();
    ...
    slideWithButtons = () => {
        ...
        this.arrowButtons.children[0].on(Input.EventType.TOUCH_START, () => {
            ...
            if (
                this.node.getComponent(PageView).content.children.length % 2 == 0 &&
                currentIndex == Math.floor(this.node.getComponent(PageView).content.children.length / 2) - 1
            ) {
    ...
    selectMode = () => {
        this.node.getComponent(PageView).content.children.forEach((element, index) => {
            ...
            if (index == 0) {
                ...
                buttons.getChildByName("InstructionButton").getChildByName("Label").getComponent(Label).string =
                    "coming soon";
            }
