...
export class Message extends Component {
  ...
  protected onLoad(): void {
    let a = photonmanager.Instance.photon_instance;
    a.messages = this;
  }
  ...
  SendMessage() {
    let id = photonmanager.Instance.photon_instance.myActor().actorNr;
    ...
    if (ReqMessage.length > 0) {
      ...
      if (
        MessageNodes.getComponent(UITransform).contentSize.width > smallText.getComponent(UITransform).contentSize.width
      ) {
        MessageBox.getComponent(UITransform).setContentSize(
      ...
      photonmanager.Instance.photon_instance.raiseEvent(Photonevents.Ghostchat, { ReqMessage, color: this.mycolor });
      this.node.getChildByName("ScrollView").getComponent(ScrollView).scrollToBottom();
    }
  }
  onetimemessage = 1;
  allmessages() {
    let totalmessages = photonmanager.Instance.photon_instance.totalmessages;
  ...
  recievedmessage(messages: any, color) {
    ...
    if (this.node.scale.x == 0) {
      photonmanager.Instance.photon_instance.totalmessages.push(messages);
    } else {
      photonmanager.Instance.photon_instance.totalmessages = [];
    }
