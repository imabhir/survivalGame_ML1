    ...
    open() {
        if (this.chest) {
            ...
            if (this.node.parent.getChildByName(this.minigame.name) == null) {
                ...
                console.log("minigame added ");

                // this.hudNode.addChild(this.minigame);
    ...
    addRigidBody() {
        ...
        this.node
            ...
            .forEach((e) => {
                ...
                console.log("e loaction ", e.x, e.y, e.height, e.width, node.position);
                let a: Rect = new Rect(e.x, e.y - e.height, 32, 32);
                let bb = this.node
                    .getChildByName("interactables")
                    .getComponent(UITransform)
                    .convertToNodeSpaceAR(new Vec3(a.x, a.y, 0));
